...
def main():
    ...
    path_to_model_to_try = r'CRNN_LOSS_NMC14_Epoch78_0.482_721.pth'  # you have to rename the model name cancelling the loss
    ...
    if path_to_model.is_file():
        ...
        path_to_model_to_train = r"CRNN_LOSS_NMC14_Epoch78_0.482_721.pth"
    ...
    for image_in_folder in list_of_test_folders:
        ...
        for image_in_list in os.listdir(current_spectrogram_path_folder):
            ...
            spectrogram_to_predict = npy_loader_test(current_spectrogram_path)[0].to(device)
            spectrogram_to_predict = torch.reshape(spectrogram_to_predict, (1, 4, 128, 94))
...
class Model(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.conv1 = nn.Conv2d(4, 32, 7, padding=3)
        self.conv2 = nn.Conv2d(32, 64, 5, padding=2)
        self.conv3 = nn.Conv2d(64, 64, 3, padding=1)

        self.gru1 = nn.GRU(256, 256, num_layers=2, batch_first=True) # 256, 256
        ...
        self.mp0 = nn.MaxPool2d((4, 1), stride=(4, 1))
    ...
    def initHidden(self, x):
        ...
        if (self.training == False) & (x.shape == (1, 4, 128, 94)):
            return torch.zeros((2, 1, 256)).to(self.device)
        else:
            return torch.zeros((2, 32, 256)).to(self.device)
    ...
    def forward(self, x, hidden):
        if self.training == False:
            ...
            while song_time < 2:
                if song_time == 1:
                    x = x.cpu().numpy()
                    if x.shape == (32, 4, 128, 94):
                        a = x[:, :, :, :47]
                        b = x[:, :, :, 47:]
                        x = np.stack([b, a], axis=-1)
                        x = np.reshape(x, (32, 4, 128, 94))
                    elif x.shape == (1, 4, 128, 94):
                        a = x[:, :, :, :47]
                        b = x[:, :, :, 47:]
                        x = np.stack([b, a], axis=-1)
                        x = np.reshape(x, (1, 4, 128, 94))

                    x = torch.from_numpy(x)
                i = 2
                while 2 <= i < 6:
                    if x.shape == (32, 4, 128, 94):
                        y = torch.from_numpy(x.cpu().numpy()[:, :, i * 12:i * 12 + 32, :]).to(self.device)
                    elif x.shape == (1, 4, 128, 94):
                        y = torch.from_numpy(x.cpu().numpy()[:, :, i * 12:i * 12 + 32, :]).to(self.device)

                    y = self.elu(self.conv1(y))
                    y = self.bn1(y)
                    y = self.mp1(y)
                    y = self.do_conv(y)
                    ...
                    y = self.do_conv(y)
                    y = y.permute(0, 3, 1, 2)
                    resize_shape = list(y.shape)[2] * list(y.shape)[3]
                    y = torch.reshape(y, (list(y.shape)[0], list(y.shape)[1], resize_shape))
                    y, _ = self.gru1(y, hidden)
                    y = y[:, -1, :]
                    y = self.do_gru(y)
                    z = self.out2(y)
                    t = self.out3(y)
                    x_list.append(self.out(y))
                    i += 1
                song_time += 1
            x = torch.stack(x_list,dim=-1).sum(dim=-1)/(4*2) # 4*2 = [max(i)-2] * max(song_time)
...
def npy_loader(path):
    ...
    slice1 = np.random.randint(2, 6)
    pitch_shift = np.random.randint(0, 6)
    song_time = np.random.randint(0, 2)

    array_to_import = np.load(path)[:2, slice1 * 12 - pitch_shift:slice1 * 12 + 32 - pitch_shift, :]

    if array_to_import.shape == (2, 32, 94):
        if song_time == 0:
            pass
        elif song_time == 1:
            ...
            array_to_import = np.reshape(array_to_import, (2, 32, 94))
        ...
        return [tensor, slice1-2, song_time]
    else:
        return [torch.zeros([4, 32, 94]), 0, song_time]
...
def npy_loader_test(path):
    ...
    song_time = 0
    array_to_import = np.load(path)[:2,:,:]
    ...
    if array_to_import.shape == (4, 128, 94):
        tensor = torch.from_numpy(array_to_import)
        return [tensor, 0, song_time]
    else:
        return [torch.zeros([4, 128, 94]), 0, song_time]
...
def trainIters(model, epochs, print_every=1, plot_every=1, learning_rate=0.00005): # 0.00008 # 0.001
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate, weight_decay=0.00008) # 001 # 00008
    ...
    for current_epoch in range(1, epochs + 1):
        ...
        for i, (X_train, y_train) in enumerate(train_loader):
            ...
            X_train, z_train, t_train = X_train[0], X_train[1], X_train[2]
            if X_train.shape[0] == batch_size:

                progress = (i * batch_size * 100) / tot_iter
                print(f"\r{progress:.3f} %", end='')

                X_train, y_train, z_train, t_train = X_train.to(device), y_train.to(device), z_train.to(device), t_train.to(device)

                model_hidden = model.initHidden(X_train)
                optimizer.zero_grad()
                y_pred, z_pred, t_pred = model(X_train, model_hidden)
                loss = criterion(y_pred, y_train)
                loss2 = criterion(z_pred, z_train)
                loss3 = criterion(t_pred, t_train)
        ...
        if current_epoch % print_every == 0:
            ...
            with torch.no_grad():
                for index_test, (inputs, labels) in enumerate(test_loader):
                    inputs, z_test, t_test = inputs[0], inputs[1], inputs[2]

                    if inputs.shape[0] == batch_size:

                        inputs, labels, z_test, t_test = inputs.to(device), labels.to(device), z_test.to(device), t_test.to(device)

                        model_hidden = model.initHidden(inputs)
                        logps, z_pred, t_pred = model.forward(inputs, model_hidden)

                        batch_loss = criterion(logps, labels)
                        batch_loss2 = criterion(z_pred, z_test)
                        batch_loss3 = criterion(t_pred, t_test)

                        test_loss += batch_loss
                        test_loss2 += batch_loss2
                        test_loss3 += batch_loss3
            ...
            print(f"Epoch {current_epoch}/{epochs}.. "
                  ...
                  f"Train loss2: {running_loss2 / ((i + 1)*print_every):.3f}.. "
                  f"Train loss3: {running_loss3 / ((i + 1)*print_every):.3f}.. "
