...
# the folder DatasetRawspectrograms
...
def main():
    ...
    list_of_test_folders = os.listdir(r"Data\\DatasetRawspectrograms\\genres_png_validation")
    ...
    for image_in_list in list_of_test_folders:
        current_spectrogram_path = pjoin(r"Data\\DatasetRawspectrograms\\genres_png_validation", f"{image_in_list}")
...
def get_png_from_wav(dir):
    ...
    dir_to_png = pjoin(f"Data\\DatasetRawspectrograms\\genres_png_training")
    dir_to_test = pjoin(f"Data\\DatasetRawspectrograms\\genres_png_test")
    dir_to_val = r"Data\\DatasetRawspectrograms\\genres_png_validation"
...
class Model(nn.Module):
    ...
    def __init__(self):
        ...
        self.gru1 = nn.GRU(1024, 64, num_layers=2, dropout=0.5, batch_first=True)
        ...
        self.mp3 = nn.MaxPool2d((2, 1), stride=(2, 1))
        self.do_conv = nn.Dropout(p=0.75) # 0.1
        self.do_gru = nn.Dropout(p=0.75) # 0.3
    ...
    def forward(self, x, hidden):
        ...
        x = self.out(x)
        # z0 = x.detach().cpu().numpy()
        x = self.logSm(x)
        # z1= x.detach().cpu().numpy()
        # check_sum_one = pd.DataFrame(z1[0,:]).applymap(lambda i: np.exp(i)).sum()


        return x

    def initHidden(self):
        return torch.zeros((2, 32, 64)).to(self.device)
...
def trainIters(model, epochs, print_every=1, plot_every=1, learning_rate=0.00005):
    device = torch.device("cuda" if torch.cuda.is_available()
                          else "cpu")
    start = time.time()
    plot_losses = []
    ...
    train_data = datasets.DatasetFolder(r"Data\\DatasetRawspectrograms\\genres_png_training",
                                        extensions="npy",
                                      loader=npy_loader)
                                      # transform=train_transforms)
    test_data = datasets.DatasetFolder(r"Data\\DatasetRawspectrograms\\genres_png_test",
                                       extensions="npy",
                                       loader=npy_loader)
    batch_size = 32
    ...
    for current_epoch in range(1, epochs + 1):
        ...
        for i, (X_train, y_train) in enumerate(train_loader):
            ...
            if X_train.shape[0]==batch_size:
                ...
                #         x_to_augment = pd.DataFrame(x_to_augment.numpy()).swifter.apply(lambda db_series: db_series.apply(lambda db: dynamic_range_compression(db))).to_numpy().astype("float")
        ...
        torch.save(model, f'CRNN_10_LOSS_{print_loss_avg:.3f}.pth')
