...
features, features_test, target, target_test = train_test_split(data, all_labels, test_size=0.1, random_state = SEED, shuffle = True)
...
# features = pd.DataFrame(features, columns=columns).sort_values(by=["filename"])
# features_validation = pd.DataFrame(features_validation, columns=columns).sort_values(by=["filename"])
# features_test = pd.DataFrame(features_test, columns=columns).sort_values(by=["filename"])
